...
* **CLAUDE-LONG-TERM-MEMORY.md** - Archive for detailed implementation history that is still true but rarely needed
* **CLAUDE-config-variables.md** - Configuration variables reference (if exists)
* **CLAUDE-temp.md** - Temporary scratch pad (only read when referenced)

## When to read memory bank files

- **Starting any work** - Read `MEMORY.md` (auto-loaded) for current state and session continuity
...
- **Before claiming work is done** - Verify against the task list and MEMORY.md that the work aligns with stated goals
...
**Trimming rule:** When removing content from `MEMORY.md` to stay under the 200-line limit, move the removed content to the appropriate `CLAUDE-*.md` file (e.g., resolved troubleshooting → `CLAUDE-troubleshooting.md`, patterns → `CLAUDE-patterns.md`, decisions → `CLAUDE-decisions.md`, detailed implementation history → `CLAUDE-LONG-TERM-MEMORY.md`). Never discard knowledge — relocate it.
